/**
 * AI Dream Analysis Schema - The Master Truth
 * 
 * This file defines the standardized output schema for all AI dream analysis in Lumi.
 * It serves as the single source of truth for Sentiments, Archetypes, and Themes.
 * 
 * DESIGN PRINCIPLES:
 * 1. Warm, Jungian, scientific yet poetic.
 * 2. Extensive enough to capture nuance (avoiding generic "happy/sad").
 * 3. Structured for UI visualization (colors, icons).
 */
...
export const getSentimentColors = (sentiment: string): { bg: string; text: string; border: string } => {
    ...
    if (["Mystery", "Transformation", "Lucidity"].includes(s)) {
        return { bg: "rgba(139, 92, 246, 0.15)", text: "#C4B5FD", border: "rgba(139, 92, 246, 0.3)" }; // Violet
    }
    if (["Nostalgia", "Longing"].includes(s)) {
